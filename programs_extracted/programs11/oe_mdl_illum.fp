...
uniform samplerCUBE	 tex_cube_norm:		TEXUNIT4;	//normalization cubemap
...
#define	USE_NORMALIZE_CUBEMAP //use fast normalize	 

#ifdef USE_NORMALIZE_CUBEMAP
	float3 normalize_func(float3 _vec)
	{
		return expand(float3(texCUBE( tex_cube_norm, _vec )));
	}
#else
	float3 normalize_func(float3 _vec)
	{
...
//	return float4(pack(normalize_func(in_data.norm)), 1.0); //texCUBE( tex_cube_norm, pack(in_data.norm) );

#ifdef DBG_OUTPUT_VNORMALS
	return float4(pack(in_data.norm), 1.0);
#endif

#ifdef DBG_OUTPUT_VEC_TO_EYE
	return float4(pack(normalize_func(in_data.to_eye)), 1.0);
#endif
	...
	normal = normalize_func(normal);	//u+se per pixel normaliation
#endif
